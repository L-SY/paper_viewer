...
type Group = { id: string; name: string; timezone: string; paper_deadline_rule: string; paper_deadline_day: number | null; paper_deadline_time: string };
type RecordRow = { group_id: string; student_id: string; official_version_id: string | null };
...
export async function POST(request: Request) {
  ...
  const now = new Date();
  const { data: groupData } = await admin.from("groups").select("id, name, timezone, paper_deadline_rule, paper_deadline_day, paper_deadline_time");
  ...
  const [{ data: memberData }, { data: recordData }, { data: profileData }, usersResult] = await Promise.all([
    ...
    monthKeys.length ? admin.from("monthly_records").select("group_id, student_id, research_month, official_version_id").in("research_month", monthKeys) : Promise.resolve({ data: [] }),
  ...
  const deliveries: Array<{ userId: string; kind: "paper"; sent: boolean }> = [];
  ...
  for (const member of members) {
    ...
    if (localMonth.day >= Math.max(1, paperDeadline - 2) && localMonth.day <= paperDeadline && !record?.official_version_id) {
      const kind = "paper" as const;
      const deliveryKind = "paper_reminder";
      ...
      const deadlineTime = shortTime(group.paper_deadline_time);
      const content = monthlyReminderEmail({ groupName: group.name, displayName: profiles.get(member.user_id) || email.split("@")[0], kind, deadline: `${localMonth.monthKey.slice(0, 7)}-${String(paperDeadline).padStart(2, "0")} ${deadlineTime}`, appUrl: `${appUrl}/student` });
