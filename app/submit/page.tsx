...
export default async function SubmitPage() {
  ...
  let initialVersions = [{ version_number: 2, original_filename: "monthly-paper-v2.pdf", size_bytes: 3984588, page_count: 12, submitted_at: "2026-07-20T22:14:00+08:00" }, { version_number: 1, original_filename: "monthly-paper-v1.pdf", size_bytes: 3565158, page_count: 11, submitted_at: "2026-07-18T19:42:00+08:00" }];
  if (session.configured && session.user && session.membership?.role === "student") {
    ...
    initialVersions = [];
    if (record) {
      const { data: versions } = await session.supabase.from("submission_versions").select("version_number, original_filename, size_bytes, page_count, submitted_at, title").eq("monthly_record_id", record.id).order("version_number", { ascending: false });
