...
export default async function TeacherDashboardPage() {
  ...
  if (session.configured && session.user && session.membership?.role === "teacher") {
    ...
    rows = members.map((member) => {
      ...
      return { id: member.user_id, name: profile?.display_name || "未命名成员", initials: (profile?.display_name || "成员").slice(0, 1), topic: `${profile?.discipline || "未填写方向"} · ${stageLabel[profile?.research_stage || "exploring"]}`, planDone: Boolean(record?.plan_text), version: versionId ? versions.get(versionId) || null : null, aiScore: aiResult?.score ?? null, aiReviewed: Boolean(aiResult), mentorScore: versionId ? teacherScores.get(versionId) ?? null : null, status: statusMap[record?.status || "missing"] || "missing", reviewId: versionId };
    });
  }
