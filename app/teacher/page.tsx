...
type DashboardRow = DemoStudent & { reviewId?: string; aiReviewed?: boolean };
type Member = { user_id: string };
type Profile = { id: string; display_name: string; discipline: string | null };
type MonthlyRecord = { id: string; student_id: string; official_version_id: string | null; status: string };
...
export default async function TeacherDashboardPage() {
  ...
  if (session.configured && session.user && session.membership?.role === "teacher") {
    ...
    const [{ data: memberData }, { data: recordData }] = await Promise.all([
      ...
      session.supabase.from("monthly_records").select("id, student_id, official_version_id, status").eq("group_id", groupId).eq("research_month", month.monthKey),
    ]);
    ...
    const [{ data: profileData }, { data: versionData }, { data: aiData }, { data: teacherData }] = await Promise.all([
      userIds.length ? session.supabase.from("profiles").select("id, display_name, discipline").in("id", userIds) : Promise.resolve({ data: [] }),
    ...
    const aiResults = new Map<string, { score: number | null }>();
    for (const review of (aiData || []) as AiScore[]) {
      if (!aiResults.has(review.submission_version_id)) {
        aiResults.set(review.submission_version_id, {
    ...
    rows = members.map((member) => {
      ...
      const aiResult = versionId ? aiResults.get(versionId) : null;
      return { id: member.user_id, name: profile?.display_name || "未命名成员", initials: (profile?.display_name || "成员").slice(0, 1), topic: profile?.discipline || "", planDone: false, version: versionId ? versions.get(versionId) || null : null, aiScore: aiResult?.score ?? null, aiReviewed: Boolean(aiResult), mentorScore: versionId ? teacherScores.get(versionId) ?? null : null, status: statusMap[record?.status || "missing"] || "missing", reviewId: versionId };
    });
  }
  ...
  const reviewed = rows.filter((row) => row.aiReviewed || row.aiScore != null).length;
  const aiMetricLabel = scored.length ? "AI均分" : "AI完成";
  const aiMetricValue = scored.length ? (scored.reduce((sum, value) => sum + value, 0) / scored.length).toFixed(1) : reviewed;
  ...
  return (
    <AppShell surface="teacher">
      ...
      <section className="month-bar" aria-label="本月流程与统计"><div className="month-stage"><span>当前阶段</span><strong>月末论文提交</strong></div><ol className="simple-flow"><li className="active">论文</li><li>AI评阅</li><li>导师评语</li></ol><dl className="inline-metrics"><div><dt>已提交</dt><dd>{submitted}</dd></div><div><dt>待评语</dt><dd>{awaiting}</dd></div><div><dt>未提交</dt><dd>{missing}</dd></div><div><dt>{aiMetricLabel}</dt><dd>{aiMetricValue}</dd></div></dl></section>
      <section className="content-section"><div className="section-heading"><h2>学生进度</h2></div><div className="table-wrap"><table className="data-table"><thead><tr><th>学生</th><th>论文版本</th><th>AI 评阅</th><th>导师评分</th><th>状态</th><th><span className="sr-only">操作</span></th></tr></thead><tbody>{rows.map((student) => <tr key={student.id}><td><div className="person-cell"><span className="avatar">{student.initials}</span><div><strong>{student.name}</strong>{student.topic && <small>{student.topic}</small>}</div></div></td><td className="mono">{student.version ? `v${student.version}` : "—"}</td><td>{student.aiScore ? <strong className="score">{student.aiScore.toFixed(1)}</strong> : student.aiReviewed ? <span className="check-text">已评阅</span> : <span className="muted-text">—</span>}</td><td>{student.mentorScore ? <strong className="score">{student.mentorScore.toFixed(1)}</strong> : <span className="muted-text">—</span>}</td><td><StatusPill status={student.status} /></td><td className="action-cell">{student.reviewId || student.id === "chen-yuhang" ? <Link className="text-link" href={`/papers/${student.reviewId || student.id}?from=current`}>查看评阅</Link> : <span className="muted-text">暂无论文</span>}</td></tr>)}</tbody></table></div></section>
    </AppShell>
  );
}
