...
export default async function StudentDashboardPage() {
  ...
  return (
    <AppShell surface="student">
      <header className="page-header"><div><div className="eyebrow">学生端 / 我的本月 {demo && <span className="demo-tag">演示</span>}</div><h1>{month.monthLabel}</h1></div></header>
      ...
      <section className="content-section"><div className="section-heading"><h2>最新反馈</h2>{versionId && <Link className="text-link" href={`/papers/${versionId}`}>查看完整评阅</Link>}</div><div className="feedback-strip"><div><span>AI 总分</span><p>{aiScore == null ? "尚未完成" : aiScore.toFixed(1)}</p></div><div><span>下一步重点</span><p>{aiScore == null ? "—" : "结合证据页码检查主要不足。"}</p></div><div><span>导师建议</span><p>{teacherComment}</p></div></div></section>
    </AppShell>
  );
}
