...
export function MonthlySubmissionForm({ initialPlan, initialTitle, initialVersions, demo }: { initialPlan: string; initialTitle: string; initialVersions: VersionItem[]; demo: boolean }) {
  ...
  return (
    <form onSubmit={submit}>
      <div className="form-layout"><div><section className="form-section"><label className="field-label" htmlFor="paper-title">论文标题 <small>上传 PDF 时必填</small></label><input id="paper-title" className="text-input full-input" name="title" defaultValue={initialTitle} placeholder="与 PDF 中的标题一致" /></section><section className="form-section"><label className="field-label" htmlFor="plan">月初计划 <small>可随研究推进继续补充</small></label><textarea id="plan" className="text-area" name="plan" defaultValue={initialPlan} placeholder="用几条清晰的目标说明本月准备做什么" /></section><section className="form-section"><div className="field-label"><span>论文 PDF</span><small>最大 30 MB · 最多 40 页</small></div><label className="upload-zone" htmlFor="paper-upload"><input id="paper-upload" name="paper" type="file" accept="application/pdf" onChange={(event) => setFileName(event.target.files?.[0]?.name || null)} /><span className="upload-icon">PDF</span><strong>{fileName || "拖入文件或点击选择"}</strong><span>上传新文件会生成新版本，不覆盖历史 PDF</span></label></section>{message && <div className={`form-message submission-message${message.success ? " success" : ""}`} role="status">{message.text}</div>}<div className="submission-actions"><button className="button button-primary" type="submit" disabled={loading}>{loading ? "保存中…" : "保存本月记录"}</button>{demo && <span>当前展示演示内容，配置后写入 Supabase。</span>}</div></div><aside><section className="aside-card"><h3>AI 评阅额度</h3><p>成功生成结果才消耗一次，API 失败不计数。</p><div className="quota"><span className="used" /><span className="used" /><span /></div><p><strong>已使用 2 / 3 次</strong></p></section><section className="aside-card"><h3>历史版本</h3><ol className="version-list">{initialVersions.length ? initialVersions.map((version) => <li key={version.version_number}><span>v{version.version_number}</span><div><strong>{version.original_filename}</strong><small>{bytesToMb(version.size_bytes)} · {version.page_count}页</small></div></li>) : <li><span>—</span><div><strong>还没有 PDF 版本</strong><small>第一次上传后会显示在这里</small></div></li>}</ol></section><section className="aside-card"><h3>可见范围</h3><p>PDF、AI 评阅和导师评语对本课题组成员公开，组外不可访问。</p></section></aside></div>
    </form>
  );
}
