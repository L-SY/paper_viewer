...
export function TeacherReviewForm({ submissionVersionId, initialScore = 8.5, initialComment = "" }: { submissionVersionId: string | null; initialScore?: number; initialComment?: string }) {
  ...
  return <form className="teacher-review-form" onSubmit={submit}><header><strong>导师独立评分</strong></header><label>总体评分 / 10<input className="text-input" name="score" type="number" min="0" max="10" step="0.1" defaultValue={initialScore} required /></label><label>评语<textarea className="text-area" name="comment" defaultValue={initialComment} placeholder="评语" required /></label>{message && <div className={`form-message${message.success ? " success" : ""}`}>{message.text}</div>}<button className="button button-primary" type="submit" disabled={loading}>{loading ? "保存中…" : "保存导师评语"}</button></form>;
}
